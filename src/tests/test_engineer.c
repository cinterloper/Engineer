...
static void engineer_test_basic(TCase *tc);

static const struct {
   const char *name;
   void (*build)(TCase *tc);
} tests[] = {
  { "basic", engineer_test_basic }
};

START_TEST(engineer_initialization)
{
   fail_if(engineer_init() != 1);

   engineer_library_call();

   fail_if(engineer_shutdown() != 0);
}
END_TEST

static void
engineer_test_basic(TCase *tc)
{
   tcase_add_test(tc, engineer_initialization);
}
