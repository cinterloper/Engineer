...
inline void AnglNEW(Eina_Inarray **key)
{
   *key = eina_inarray_new(sizeof(Angl), 0);
}

inline void AnglFREE(Eina_Inarray **key)
{
   eina_inarray_free(*key);
}

inline void AnglPUSH(Eina_Inarray **key, Angl *input)
{
   eina_inarray_push(*key, input);
}

inline void AnglPOP(Eina_Inarray **key)
{
